...
def plot_config(filename,beta,num):
	...
	bener = np.loadtxt("outputs/Boltzmann0.400000.dat")
	#plt.plot(cnifgs[:num+1,0],cnifgs[:num+1,1])
	x = np.arange(512)
	E = np.exp(2.0*beta*bener[:,1])
	ey = savgol_filter(E,51,1)
	y = savgol_filter(np.bincount(cnifgs[:,1]),51,1)
	y=np.bincount(cnifgs[:,1])
	plt.plot(x,1.0*y/np.sum(y))
	plt.plot(x,E/np.sum(E),c='r')
...
def Getmandplotm(filename,beta):
	...
	plt.savefig("plot/sim_mPlot_log"+repr(beta)+".png")
	#plt.show()
	plt.close()
	...
	plt.savefig("plot/sim_Eplot_log_"+repr(beta)+".png")
	#lt.show()
	plt.close()
	#return np.sum(m[::N_eff])/N_eff, err
plot_config("outputs/Whichconfig_500.dat",0.4,500)
def mvsbeta():
...
#Getmandplotm("simann_log_out_2",0.2)
...
# pk.dump((m),open("m2savefor20.pk","wb"))
# plt.plot(betiter,m)
# plt.xlabel("beta")
# plt.ylabel("magnetization <m^2>")
# plt.title("<m^2> vs beta plot for 2D 9x9 Ising model")
# plt.savefig(currdir+"/plot/m2vsbeta9x9.png")
# plt.show()
