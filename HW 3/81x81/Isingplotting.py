...
def svngcgrain(fname):
	cg = np.loadtxt("sanp/"+fname)
...
def Getmandplotm(filename,beta):
	...
	plt.savefig("plot/mPlot"+repr(beta)+".png")
	#plt.show()
	plt.close()
	...
	plt.savefig("plot/Eplot"+repr(beta)+".png")
	#lt.show()
	plt.close()
...
def plotM(sshot,beta,cg=0):
	...
	for i in range(100):
		...
		Mvals.append(1.0*redising.sum()/redising.size)
		mvals.append((1.0*redising.sum()/redising.size)**2)
...
betiter = np.linspace(1,5,5,dtype=int)
#snapcoarsegrain("snap_dats/snap_1_12.dat",1.2)
...
# merr = []
# for i in betiter:
# 	#svngcgrain("out_"+repr(i))
# 	#np.savetxt("cgout_"+repr(i),coarse_grain("out_"+repr(i)))
# 	val, err = Getmandplotm("cgout_"+repr(i),1.0*i/10.0)
# 	m.append(val)
# 	merr.append(err)

# betiter=1.0*betiter/10.0
# pk.dump((m),open("m2savefor81cg27.pk","wb"))
# plt.plot(betiter,m)
# plt.xlabel("beta")
# plt.ylabel("magnetization <m^2>")
# plt.title("<m^2> vs beta plot for 2D 27x27 cgrained Ising model")
# plt.savefig(currdir+"/plot/m2vsbeta27x27cgrain.png")
# plt.show()
# M = []
# for i in betiter:
# 	M.append(plotM("snap_"+repr(i),1.0*i/10.0,1))

# pk.dump(M,open("msqrdagsaveforcgrain81.pk","wb"))
# plt.plot(1.0*betiter/10.0,M)
# plt.xlabel("beta")
# plt.ylabel("<m^2>")
# plt.title("<m^2> for different beta values for coarse grained 27x27")
# plt.savefig("msq81x81.png")
# plt.show()
# plt.close()
snapcoarsegrain("snap_dats/snap_9_10.dat",0.9)
